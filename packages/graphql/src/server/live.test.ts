...
it('works with static values', async () => {
...
it('works with lists', async () => {
  ...
  const resolver = createQueryResolver(({object}) => ({
    me: object('Person', {
      name: 'Chris',
      pets: [object('Dog', {name: 'Winston'}), object('Dog', {name: 'Molly'})],
    }),
  }));
...
it('works with inline fragments on a concrete type', async () => {
  const query = parse(`
...
it('works with iterators in nested objects', async () => {
  const spy = jest.fn();
...
it('works with nested iterators', async () => {
  const spy = jest.fn();
    ...
    me: object('Person', {
      ...
      pets: () =>
        iterate([
          ...
          [
            object('Dog', {name: 'Molly'}),
            object('Dog', {name: 'Winston', age: () => iterate([8, 9, 10])}),
          ],
        ]),
    }),
  }));
