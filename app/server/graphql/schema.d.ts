import {
  ...
  type EpisodeEndpointSelector,
  type EpisodeRangeSelector,
  type EpisodeSelectionSelector,
} from '@watching/api';
export interface Query {
  __typename: 'Query';
  apps(variables: Record<string, never>): App[];
  app(variables: {readonly id: string}): App | null;
  ...
  list(variables: {readonly id: string}): List | null;
  watchLater(variables: Record<string, never>): List;
  version(variables: Record<string, never>): ApiVersion | null;
  search(variables: {readonly query: string}): SearchResults;
  subscription(variables: {readonly id: string}): SeriesSubscription | null;
  subscriptions(variables: Record<string, never>): SeriesSubscription[];
  watch(variables: {readonly id: string}): Watch | null;
  watchThrough(variables: {readonly id: string}): WatchThrough | null;
...
export interface ClipsExtensionInstallation {
  ...
  ): ClipsExtensionPointSupportLoading | null;
}
...
export interface ClipsExtensionPointSupportCondition {
  ...
  series(
    variables: Record<string, never>,
  ): ClipsExtensionPointSupportSeriesCondition | null;
}
...
export interface ClipsExtensionPointSupportLoading {
  ...
  ui(
    variables: Record<string, never>,
  ): ClipsExtensionPointSupportLoadingUi | null;
}
...
export type ClipsExtensionSettingsField =
...
export type ClipsExtensionSettingsString =
...
export interface Mutation {
  ...
  deleteApp(variables: {readonly id: string}): DeleteAppPayload;
  updateApp(variables: {
    readonly id: string;
  ...
  installApp(variables: {readonly id: string}): InstallAppPayload;
  ...
  disconnectGoogleAccount(
    variables: Record<string, never>,
  ): DisconnectGoogleAccountPayload;
  ...
  startPasskeyCreate(
    variables: Record<string, never>,
  ): StartPasskeyCreatePayload;
  finishPasskeyCreate(variables: {
    readonly credential: JSON;
  }): FinishPasskeyCreatePayload;
  deletePasskey(variables: {readonly id: string}): DeletePasskeyPayload;
  ...
  deleteSeries(variables: {readonly id: string}): DeleteSeriesPayload;
  ...
  watchLater(variables: {readonly seriesId?: string | null}): WatchLaterPayload;
  ...
  deleteWatch(variables: {readonly id: string}): DeleteWatchPayload;
}
...
export interface WatchThrough {
  ...
  nextSeason(variables: {readonly inSelection?: boolean | null}): Season | null;
